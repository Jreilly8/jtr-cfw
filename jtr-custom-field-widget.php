<?php
/*
PLUGIN NAME: JTR Custom Field Widget

DESCRIPTION: Displays the values of specified custom field keys, allowing post- and page-specific meta content in your sidebar, or a default string.
...
JTR Field Widget is free software: you can redistribute it and/or
modify it under the terms of the GNU General Public License as
published by the Free Software Foundation, either version 3 of
the License, or (at your option) any later version.
JTR Custom Field Widget is distributed in the hope that it will be useful,
...
// Function for the RPM National Custom Field Widget
function wp_widget_jtr_custom( $args, $widget_args = 1 ) {
	...
	$default = apply_filters( 'widget_text', $options[$number]['default'] );
...
// Function for the RPM National Custom Field Widget options panels
function wp_widget_jtr_custom_control($widget_args) {
	...
	if ( !$updated && !empty($_POST['sidebar']) ) {
		...
		foreach ( $this_sidebar as $_widget_id ) {
			...
			if ( 'wp_widget_jtr_custom' == $wp_registered_widgets[$_widget_id]['callback'] && isset($wp_registered_widgets[$_widget_id]['params'][0]['number']) ) {
		...
		foreach ( (array) $_POST['widget-custom-field'] as $widget_number => $widget_custom_field ) {
			...
			if ( current_user_can('unfiltered_html') ) {
				...
				$title = stripslashes($widget_custom_field['title']);
			} else {
				$text = stripslashes(wp_filter_post_kses($widget_custom_field['text']));
				$title = strip_tags(stripslashes($widget_custom_field['title']));
			}
			if ( current_user_can('unfiltered_html') ) {
				$default = stripslashes($widget_custom_field['default']);
			} else {
				$default = stripslashes(wp_filter_post_kses($widget_custom_field['default']));
			}
			// We're saving as an array, so save the options as such
			$options[$widget_number] = compact( 'key', 'title', 'text','default' );
		}
	...
	// Otherwise, this widget has stored options to return
		...
		$key    = esc_attr($options[$number]['key']);
		$title  = format_to_edit($options[$number]['title']);
		$text   = format_to_edit($options[$number]['text']);
		$default   = format_to_edit($options[$number]['default']);
	}
	// Our actual widget options panel
?>
	<p><?php printf( __( 'Enter the custom field key <a href="%s">[?]</a>  to locate in single posts/pages. When found, the corresponding value is wrapped by HTML BEFORE and HTML AFTER, or the DEFAULT VALUE is handled in the same fashion.', 'cf_widget' ), 'http://codex.wordpress.org/Using_Custom_Fields' ) ?></p>
	
	<p>
		<label for="custom-field-title-<?php echo $number; ?>"><?php _e( 'HTML BEFORE:', 'cf_widget' ) ?></label>
		<textarea id="custom-field-title-<?php echo $number; ?>" name="widget-custom-field[<?php echo $number; ?>][title]" class="widefat" rows="5" cols="20"><?php echo $title; ?></textarea>
	</p>
	<p>
		...
		<p>
		<label for="custom-field-default-<?php echo $number; ?>"><?php _e( 'Custom Field Default (required):', 'cf_widget' ) ?></label>
		<input id="custom-field-default-<?php echo $number; ?>" name="widget-custom-field[<?php echo $number; ?>][default]" class="widefat" type="text" value="<?php echo $default; ?>" />
	</p>
	<p>
		<label for="custom-field-text-<?php echo $number; ?>"><?php _e( 'HTML AFTER:', 'cf_widget' ) ?></label>
	...
	<p><?php printf( __( 'Coded by <a href="%s">Joseph Reilly</a>.', 'cf_widget' ), 'https://github.com/Jreilly8' ) ?></p>
	<p>
<?php
	// And we're finished with our widget options panel
}
// Function to add widget option table when activating this plugin
function wp_widget_jtr_custom_activation() {
	add_option( 'widget_custom_field', '', '', 'yes' );
}
// Function to delete widget option table when deactivating this plugin
function wp_widget_jtr_custom_deactivation() {
	delete_option('widget_custom_field');
}
// Function to initialize the RPM National Custom Field Widget: the widget and widget options panel
function wp_widget_jtr_custom_register() {
	...
	$name = __( 'JTR CFW', 'cf_widget' );
	// Assume we have no widgets in play.
	$id = false;
	// Since we're dealing with multiple widgets, we must register each accordingly
	foreach ( array_keys($options) as $o ) {
		...
		wp_register_sidebar_widget( $id, $name, 'wp_widget_jtr_custom', $widget_ops, array( 'number' => $o ) );
		wp_register_widget_control( $id, $name, 'wp_widget_jtr_custom_control', $control_ops, array( 'number' => $o ) );
	}
	// Create a generic widget if none are in use
	if ( !$id ) {
		// Register the widget and then the widget options menu
		wp_register_sidebar_widget( 'custom-field-1', $name, 'wp_widget_jtr_custom', $widget_ops, array( 'number' => -1 ) );
		wp_register_widget_control( 'custom-field-1', $name, 'wp_widget_jtr_custom_control', $control_ops, array( 'number' => -1 ) );
	}
}
...
register_activation_hook( __FILE__, 'wp_widget_jtr_custom_activation' );
register_deactivation_hook( __FILE__, 'wp_widget_jtr_custom_deactivation' );
// Allow localization, if applicable
load_plugin_textdomain('cf_widget');
// Initializes the function to make our widget(s) available
add_action( 'init', 'wp_widget_jtr_custom_register' );
// End
